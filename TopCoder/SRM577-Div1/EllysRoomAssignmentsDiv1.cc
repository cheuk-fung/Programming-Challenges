...
class EllysRoomAssignmentsDiv1 {
    ...
    public:
        ...
        {
            string Rstr;
            for (int i = 0; i < SIZE(ratings); i++) Rstr += ratings[i];
            istringstream is(Rstr);
            vector<int> R;
            int r;
            while (is >> r) R.push_back(r);
            int elly = R[0];
            ...
            double res = elly;
            for (int i = 0; i + rooms <= n; i += rooms) {
                bool isEllyHere = false;
                int sum = 0;
                ...
                if (!isEllyHere) res += sum * 1.0 / rooms;
            }
            ...
            debug(res);
            double realres = res / m * (rooms - (n - l)) / rooms;
            for (int i = l; i < n; i++) {
                realres += (res + R[i]) / (m + 1) / rooms;
            }
            return realres;
        }
};
...
int main(int argc, char* argv[])
{
	if (argc == 1) 
	{
		cout << "Testing EllysRoomAssignmentsDiv1 (250.0 points)" << endl << endl;
		for (int i = 0; i < 20; i++)
		{
		...
		switch (_tc)
		{
			case 0:
			{
				string ratings[] = {"1924 1242 1213 1217 2399 1777 2201 2301 1683 2045 ", "1396 2363 1560 2185 1496 2244 2117 2207 2098 1319 ",
				                    ...
				                    "1947 1202 1203"};
				_expected = 1821.3291005291007;
				_received = _obj.getAverage(vector <string>(ratings, ratings+sizeof(ratings)/sizeof(string))); break;
			}
			case 1:
			{
