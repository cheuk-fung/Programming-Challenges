...
class SegTree {
    ...
    public:
        ...
        {
            if (c <= heap[p].a && heap[p].b <= d) {
                heap[p].v += v;
                heap[p].max = fmax(heap[p * 2].max, heap[p * 2 + 1].max) \
                              + heap[p].v;
                return ;
            }

            if (c < (heap[p].a + heap[p].b) / 2) insert(c, d, v, p * 2);
            if (d > (heap[p].a + heap[p].b) / 2) insert(c, d, v, p * 2 + 1);
            heap[p].max = fmax(heap[p * 2].max, heap[p * 2 + 1].max) \
                          + heap[p].v;
        }
